...
This guide shows the commands used to set up your four Clean Architecture projects and define the correct dependencies using the .NET CLI, based on your solution name: **`OrderProcessing.sln`**.
...
| **Optional**               | **Domain**                  | `dotnet add OrderProcessing.Api/ reference OrderProcessing.Domain/`            |

***Note:*** The `Domain` project has no outgoing dependencies, preserving its status as the core of the application. The API layer should **never reference Infrastructure directly**; it should access services via **Application layer interfaces** and rely on **dependency injection** for concrete implementations.
